...
1. Create `inventories/providers/<provider>/` with:
   ...
   - `group_vars/all/<provider>.yml` — provider-specific vars (region, server type, etc.)
...
- `hcloud_topology` count map drives server creation — no static host list needed.
- Server labels (`cluster=kube1`, `role=hybrid`) are the bridge between provisioning and
  the dynamic inventory plugin.
- `hcloud.sops.yaml` holds the API token; the `module_defaults` group key
  `group/hetzner.hcloud.all: { api_token: ... }` injects it into every hcloud task.
