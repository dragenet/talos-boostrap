...
The user runs these in their own terminal — agents must not run live cluster
operations.
...
The token needs `Zone:DNS:Edit` scope on `dragenet.dev` (or whichever zone
the ClusterIssuers challenge against — see the `apiTokenSecretRef` in the
issuer manifests).
...
A `Certificate` request against either issuer (e.g. a wildcard
`*.dragenet.dev`) is the real end-to-end test — until that exists, the
"challenge would solve" status can only be inferred from the issuer's
`Registered` condition. The data-path `Certificate` is part of the deferred
follow-up.
...
- **No `Certificate` resources exist** — the data path is intentionally out
  of scope. No wildcard `*.dragenet.dev`, no per-service certs. Adding a
...
- **No LB Service / public IP** — the Hetzner CCM ships with
  `networking.enabled: false` (ADR-005), so the EG controller cannot
  provision a Hetzner LB Service automatically. Production traffic
  ingress must come through a Hetzner LB pointing at Envoy proxy pods, or
...
- **EG bundled Gateway API CRDs are NOT being skipped** (a `helm template`
  ...
  bundled CRDs (`helm template envoy-gateway oci://docker.io/envoyproxy/gateway-helm --version v<X.Y.Z> | grep -c "kind: CustomResourceDefinition"`).
  A non-zero count means the CRDs will conflict with the
  `gateway-api/<channel>` catalog entry.
