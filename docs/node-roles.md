...
> `config/clusters/kube1/`.** Specifically: `cluster.yaml` (provider/features/versions),
> `nodes.yaml` (node list/roles), and the Talos-shaped patch overlays under
> `config/clusters/kube1/talos/*` (per-role and per-node). Committed generated files
> start with `# Generated by render-config; do not edit.` Edit the cluster inputs
> and re-render — never hand-edit the generated files. The legacy
> `config/overrides/` tree still exists as a migration fallback but is no longer
...
`config/clusters/kube1/cluster.yaml` `cluster.nodeRoleShort`):
```yaml
node_role_short:
  controlplane: cp
  hybrid: hb
...
`config/clusters/kube1/cluster.yaml` `hcloud.topology`). Per-node role assignment lives in
`config/clusters/kube1/nodes.yaml`:
```yaml
hcloud_topology:
  controlplane: 0
  hybrid: 3
...
To change counts:
1. Edit `hcloud.topology` in `config/clusters/kube1/cluster.yaml` (and/or per-node
   `node_role` entries in `config/clusters/kube1/nodes.yaml`).
