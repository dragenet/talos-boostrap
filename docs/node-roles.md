...
> `playbooks/render-config.yml` from `config/overrides/cluster.yaml`.** The committed
> files start with `# Generated by render-config; do not edit.` Edit
> `config/overrides/cluster.yaml` and re-render — never hand-edit the generated files.
...
`config/overrides/cluster.yaml` `cluster.nodeRoleShort`):
```yaml
node_role_short:
  controlplane: cp
  hybrid: hb
...
`config/overrides/cluster.yaml` `hcloud.topology`):
```yaml
hcloud_topology:
  controlplane: 0
  hybrid: 3
...
To change counts:
1. Edit `hcloud.topology` in `config/overrides/cluster.yaml`.
