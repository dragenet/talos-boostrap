---
title: Upgrades
description: How to upgrade Talos OS version, change extensions (schematic), and perform config-only updates without an OS reboot.
type: guide
audience: [operator]
tags: [upgrades, talos, kubernetes]
status: stable
created: 2026-07-01
updated: 2026-07-01
related:
...
This guide covers the two upgrade paths available in this template: config-only changes (no OS upgrade) and full OS version or schematic upgrades.
...
`talos_version` + `talos_schematic_id` labels already exists in the cloud provider.
To force a rebuild, delete the existing snapshot from the cloud console
(or via the provider CLI), then re-run `image-talos.yml`.
...
On nodes that booted from the pre-built snapshot this is normally a no-op.
...
```
The previous schematic ID is in git history (`config/clusters/<cluster>/cluster.yaml` for the version/extensions that produced it) or the provider snapshot labels.
