---
title: Storage — OpenEBS Local PV LVM
description: Day-2 operational runbook for the OpenEBS LVM LocalPV local-storage tier: enabling, verifying, resizing, and troubleshooting.
type: guide
audience: [operator, user]
tags: [storage, openebs, kubernetes]
status: stable
created: 2026-07-01
updated: 2026-07-01
related:
  - ../adr/0009-openebs-local-lv-lvm.md
---

# Storage — OpenEBS Local PV LVM
...
`../adr/0009-openebs-local-lv-lvm.md` — this document is the day-2 procedure.
...
> `../adr/0009-openebs-local-lv-lvm.md` for the trade-off.
...
> `helmrelease.yaml` postRenderer shell script.
...
```bash
# All nodes host an lvm-node pod.
...
- `openebs-lvm-localpv-node-xxxx` pod `Running` on each node.
...
metadata:
  name: openebs-lvm-test
spec:
  nodeName: <cluster-name>-hb1                # pin to one of the hybrid nodes
  containers:
    - name: app
      image: busybox:1.37
...
- **Architectural background** — see `../adr/0009-openebs-local-lv-lvm.md`.
