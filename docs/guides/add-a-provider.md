...
description: How to add a new cloud or bare-metal provider by satisfying the inventory contract consumed by the cluster playbooks.
type: guide
audience: [contributor, operator]
tags: [providers, ansible]
status: stable
created: 2026-07-01
updated: 2026-07-01
related:
  - ../concepts/provider-portability.md
---

# Add a Provider

This guide explains how to add a new cloud or bare-metal provider to this template. The cluster layer (`talos_config`, `talos_upgrade`, `flux_bootstrap`) is provider-neutral — it consumes a contract of per-node hostvars and Ansible groups. Any provider that satisfies the contract can drive the cluster playbooks without modification.
...
> uses DHCP on the private interface plus a static VIP, which converges with the provider's
...
are unset and the cluster playbooks fail. See `../adr/0014-node-discovery-dynamic-inventory.md`
for the rationale behind dynamic inventory over static host lists.
...
- Server labels (`cluster=<cluster_name>`, `role=hybrid`) are the bridge between provisioning and
