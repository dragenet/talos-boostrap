...
│   │   │   ├── _components/         # TEMPLATE-OWNED catalog (sparse today — no feature entries yet)
│   │   │   ├── base/                # GENERATED — enabled components for this cluster (resources: [] today)
...
- **Working — config compiler (ADR-010 / 011 / 012 / 013):** `playbooks/render-config.yml` + the `config_render` role are built. `config/defaults/` (template-owned) is deep-merged with `config/overrides/` (user-owned) — overrides win per-key — then validated and rendered into the existing `inventories/.../group_vars/`, the layered Talos patch set under `infra/talos/patches/generated/{all,controlplane}/`, and the render-owned `flux/infrastructure/controllers/base/kustomization.yaml` (sparse today). Render check mode (`-e config_render_check=true` or `--tags check`) renders to a temp dir and diffs against the committed generated tree. Generated files start with `# Generated by render-config; do not edit.` — edit the overrides, not the outputs.
- **Not built yet — cluster features:** Flux is *not* bootstrapped — `flux_bootstrap` is an empty stub and `flux_repo_url` is unset in `config/overrides/cluster.yaml`. Cilium is *not* installed (the "Cilium pre-Flux" step in `bootstrap-flux.yml` is planned, not implemented). Hetzner CCM and Hetzner CSI are *not* installed. cert-manager, OpenEBS, and ingress are *not* installed. The `flux/infrastructure/controllers/_components/` catalog has only directory placeholders, and the render-owned `base/kustomization.yaml` is `resources: []`; `flux/infrastructure/configs` and `flux/apps` are empty `resources: []` placeholders. OpenEBS raw-volume validation against `RawVolumeConfig` and the `talosctl` deviceSelector probe (ADR-013) are also deferred.
...
- Cilium must be installed before Flux reconciliation starts — it is a CNI dependency that prevents pods from scheduling. (Cilium install is *not yet built* — see Implementation status.)
...
- `flux/infrastructure/controllers/` follows the catalog / render-owned base / user overlay split from ADR-010/012: `_components/` is the template-owned catalog (sparse today), `base/kustomization.yaml` is render-generated from `config/overrides/`, and `patches/` + the top-level `kustomization.yaml` are user-owned.
