...
| TLS | cert-manager — controller only in the catalog (CA/DNS-agnostic). ClusterIssuers and Certificates live in the per-cluster configs overlay. |
...
| Ingress | Envoy Gateway — Gateway API controller (`gateway.networking.k8s.io/v1`), GatewayClass `envoy-gateway`. Chart-bundled Gateway API CRDs disabled; the render-selected `gateway-api/<channel>` base is the single source of truth. |
...
│   │   └── cluster.yaml
│   ├── schemas/                  # JSON Schemas — validate cluster.yaml / nodes.yaml inputs
│   │   ├── cluster-input.schema.json
│   │   └── nodes.schema.json
│   ├── clusters/                 # USER-OWNED — selected-cluster source of truth (ADR-016)
│   │   └── kube1/                # one dir per cluster; render loads config/clusters/<name>/ FIRST
│   │       ├── cluster.yaml      #   cluster_name, VIP, endpoint, kubernetes_version, features, cni, ccm, flux
│   │       ├── nodes.yaml        #   per-node hardware + role + host name + extensions
│   │       └── talos/            #   per-cluster Talos patch manifests → rendered into infra/talos/patches/generated/
│   │           ├── all.yaml
│   │           ├── controlplane.yaml
│   │           ├── worker.yaml
│   │           └── nodes/.gitkeep
│   ├── talos/                    # TEMPLATE-OWNED — base Talos manifests + component patches
│   │   ├── base/                 #   (all/controlplane/worker) rendered for every cluster
│   │   └── components/           #   opt-in component patches (cilium.yaml, external-cloud-provider.yaml)
│   └── overrides/                # LEGACY FALLBACK (pre-ADR-016) — read ONLY when config/clusters/<name>/ is absent
│       ├── cluster.yaml
│       └── nodes.yaml
...
│   │   ├── playbooks/
│   │   │   ├── render-config.yml     # Local: render config/* into generated group_vars + Talos patches + Flux selected
...
│   └── talos/
│       ├── patches/
│       │   └── generated/        # GENERATED Talos patch layers (ADR-013: structured helpers + raw fragments)
...
│   │       └── apps.yaml
│   ├── infrastructure/
│   │   ├── controllers/          # Four-tier overlay: catalog / generated-selected / common / per-cluster (ADR-017)
│   │   │   ├── _components/         # TEMPLATE-OWNED catalog (cilium + providers/hcloud/ccm on kube1; future csi, cert-manager, ingress, openebs)
│   │   │   ├── generated/selected/  # GENERATED by render-config — enabled components for this cluster (cilium + hcloud-ccm on kube1)
│   │   │   ├── overlays/common/     # REPO-WIDE — user patches shared across all clusters
│   │   │   ├── overlays/clusters/kube1/  # PER-CLUSTER — cluster-specific patches for kube1
│   │   │   └── kustomization.yaml   # USER-OWNED overlay (references overlays/clusters/kube1)
│   │   └── configs/              # Four-tier overlay: base / common / per-cluster (ADR-017)
│   │       ├── base/                # TEMPLATE/SHARED — empty until config resources are enabled
│   │       ├── overlays/common/     # REPO-WIDE — shared config customizations
│   │       ├── overlays/clusters/kube1/  # PER-CLUSTER — cluster-specific config overrides
│   │       └── kustomization.yaml   # USER-OWNED overlay (references overlays/clusters/kube1)
│   └── apps/                     # Four-tier overlay: base / common / per-cluster (ADR-017)
│       ├── base/                    # TEMPLATE/SHARED — empty until workload manifests are added
│       ├── overlays/common/         # REPO-WIDE — shared workload customizations
│       ├── overlays/clusters/kube1/ # PER-CLUSTER — cluster-specific workload overrides
│       └── kustomization.yaml       # USER-OWNED overlay (references overlays/clusters/kube1)
│
...
- **Working — config compiler (ADR-010 / 011 / 012 / 013 / 017):** `playbooks/render-config.yml` + the `config_render` role are built. `config/defaults/` (template-owned) is deep-merged with `config/clusters/<name>/` (user-owned, ADR-016 selected-cluster source of truth) — overrides win per-key — then validated and rendered into the existing `inventories/.../group_vars/`, the layered Talos patch set under `infra/talos/patches/generated/{all,controlplane}/`, and the render-owned `flux/infrastructure/controllers/generated/selected/kustomization.yaml` (cilium + hcloud-ccm on kube1). Render check mode (`-e config_render_check=true` or `--tags check`) renders to a temp dir and diffs against the committed generated tree. Generated files start with `# Generated by render-config; do not edit.` — edit the overrides, not the outputs.
- **Working — pre-Flux + Flux (Cilium, hcloud CCM, Flux adoption):** `playbooks/bootstrap-flux.yml` runs the `flux_bootstrap` role, which is now a real implementation (preflight → namespaces → cilium → hcloud_ccm → flux) — not a stub. The role installs the **Cilium** Helm release into the `cilium-system` namespace and the **hcloud Cloud Controller Manager** Helm release into the `hcloud-ccm-system` namespace, then runs `flux bootstrap git` to install Flux controllers in `flux-system` and adopt the same releases from the render-generated `generated/selected/kustomization.yaml`. The render-owned `generated/selected/kustomization.yaml` references `../../_components/cilium` and `../../_components/providers/hcloud/ccm`; both catalog entries have matching `Namespace` + `HelmRepository` + `HelmRelease` (chart versions pinned). The Talos all-node patch `infra/talos/patches/generated/all/10-cloud-provider.yaml.j2` is generated and applied to set `cluster.externalCloudProvider.enabled: true` (so the CCM can set `spec.providerID` and clear the uninitialized taint). The pre-Flux component list (`bootstrap_pre_flux_components`) is computed by the compiler from `cni.name` + `cloud_controller_manager.enabled` + `provider.name`; the hcloud provider renders `[cilium, hcloud-ccm]`, the manual provider renders `[cilium]` (no CCM, no external cloud provider patch).
- **Working — cert-manager + Envoy Gateway (TLS + L7 controllers, data path deferred):** both catalog entries are built and Flux-managed — no pre-Flux Ansible install, no adoption handshake. The render compiler extends `config_render_flux_features` + `config_render_flux_selected_resources` to include the three new entries when `features.certManager` and `features.ingress == 'envoy-gateway'` are set; the channel knob `ingress.gatewayApiChannel` (default `standard`, also `experimental`) selects exactly one of `gateway-api/standard` vs `gateway-api/experimental`. Catalog sources are all pinned: cert-manager chart `1.20.3` from `oci://quay.io/jetstack/charts` with `crds.enabled: true` and `crds.keep: true` (controller only — CA/DNS-agnostic, namespace `cert-manager` deliberately non-`-system` per the chart default); Envoy Gateway chart `gateway-helm` v1.8.1 from `oci://docker.io/envoyproxy` with `install.crds: Skip` / `upgrade.crds: Skip` (bundled CRDs disabled, namespace `envoy-gateway-system`); Gateway API CRDs via the `gateway-crds-helm` chart pinned at the **same v1.8.1** as the EG controller for CRD compatibility, with `crds.gatewayAPI.channel: "standard"` (or `"experimental"`) and the EG controller HelmRelease declaring `dependsOn: envoy-gateway-crds` to avoid "no matches for kind GatewayClass" on first apply. A `GatewayClass envoy-gateway` (controllerName `gateway.envoyproxy.io/gatewayclass-controller`) is part of the EG base — controller-level binding only. Two `ClusterIssuer` resources (`letsencrypt-staging`, `letsencrypt-prod`, ACME + Cloudflare DNS-01) live in the user-owned `flux/infrastructure/configs/overlays/clusters/kube1/` overlay (ADR-017 per-cluster) and reference the token Secret `cloudflare-api-token/api-token` **by name**; the Secret is not committed and Flux SOPS-with-age decryption is not yet wired. **Known gaps** (deliberate, deferred follow-up): Cloudflare token Secret is user-provisioned manually (DNS-01 challenges will not solve until it exists, though ACME account registration still reaches Ready); no wildcard `Certificate`, no `Gateway`, no `HTTPRoute`, no host-network `EnvoyProxy` — the data-path follow-up slice. See `docs/cert-manager-ingress.md` for the verification runbook. As with the rest of the Working section, **live cluster state is unverified** — the code is built, lint-clean, and passes `kustomize build` / `flux build` / `helm template` / `kubeconform` / `yamllint` review, but no live reconciliation has been observed.
- **Still deferred:** Hetzner CSI is *not* installed — `features.hcloudCsi: false` and no catalog entry. OpenEBS LVM LocalPV is enabled (`features.openebs: true`) but its runbook lives in `docs/openebs-local-storage.md`; the cert-manager + Envoy Gateway data path (Gateway, wildcard Certificate, HTTPRoute, host-network EnvoyProxy, Cloudflare Secret) is deferred to a follow-up slice. `flux/apps/` is still an empty `resources: []` placeholder. The `talosctl` deviceSelector probe (ADR-013) for replacing the current `interface: eth1` form is also deferred. **Live cluster state is unverified** — every item in this "Working" section has been reviewed, but the user has not yet run `bootstrap-flux.yml` against a live cluster, so "working" means "the code is built and passes review/validation" not "we have observed it running".
...
The render compiler is the source of truth: edit `config/clusters/<name>/` (e.g. `config/clusters/kube1/`), render, then push.

```bash
# 1. Edit the user-owned input — config/clusters/<name>/cluster.yaml (e.g. config/clusters/kube1/cluster.yaml; or nodes.yaml
#    for per-node hardware). NEVER edit files marked
...
### Talos & Flux
- The machine config is layered (ADR-013) from a render-generated patch set under `infra/talos/patches/generated/{all,controlplane}/` (with optional per-node layers). The renderer reads `config/clusters/<name>/cluster.yaml` and `nodes.yaml` (ADR-016 selected-cluster source; `config/overrides/` is a legacy fallback) — never hand-edit the generated patch files; the next render overwrites them. Final machine configs are rendered into the gitignored `secrets/` dir by `talosctl gen config` + the patch set; never commit them — they embed the cluster CA and keys.
...
- Machine configs are generated via `talosctl gen config` and then composed with the layered patch set in `infra/talos/patches/generated/` (ADR-013). Never commit the rendered machine configs that contain secrets.
- The patch set itself is generated by `playbooks/render-config.yml` from `config/clusters/<name>/cluster.yaml` + `nodes.yaml` (ADR-011, ADR-016). Edit the overrides, render, then push with `bootstrap-talos.yml` — hand-editing the generated patch files is a no-op, the next render overwrites them.
...
- `flux/infrastructure/controllers/` follows the catalog / generated-selected / common / per-cluster split from ADR-017: `_components/` is the template-owned catalog (cilium + providers/hcloud/ccm on kube1 today; future csi, cert-manager, ingress, openebs), `generated/selected/kustomization.yaml` is render-generated from `config/clusters/<name>/` (ADR-016 selected-cluster config), `overlays/common/` is repo-wide user patches, and `overlays/clusters/kube1/` + the top-level `kustomization.yaml` are per-cluster user-owned.
...
When `cluster.externalCloudProvider.enabled: true` is applied via the generated Talos patch (`infra/talos/patches/generated/all/10-cloud-provider.yaml.j2`), Talos runs kubelet and kube-controller-manager with `--cloud-provider=external` and Kubernetes taints every node with `node.cloudprovider.kubernetes.io/uninitialized:NoSchedule`. Only a CCM can clear this taint. Until it does, no workloads schedule — including Flux's own controllers — so the CCM *must* run before `flux bootstrap`.
